...
::
if not exists %~dp07za.exe goto unzip
"%~dp07za.exe" x *.zip -o*
:unzip
"c:\Program Files (x86)\Tanium\Tanium Client\StdUtils\7za.exe" x *.zip -o*
:: Setting variables for command-line options
set case=%tanium
set c_drive=%c
set selection=%3
::use to move data off client
::set collect=%"c:\Program Files (x86)\Tanium\Tanium Client\tools\IR"
...
if not exist %c_drive%:\Data-%case% (
	echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create the case output folder Data-%case%
	echo:
	mkdir %c_drive%:\Data-%case%
	)
...
:main
	...
	if not exist %c_drive%:\Data-%case%\%computername%-%timestamp% (
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create the output directory %computername%-%timestamp%
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create the output directory %computername%-%timestamp% >> %c_drive%:\Data-%case%\Collection.log
		mkdir %c_drive%:\Data-%case%\%computername%-%timestamp%
		)
:: ****************************************************************************************************************************
:: Preserving Files
::
	...
	:: Creates directory to store preserved prefetch files
		echo %DATE% %TIME% - Running mkdir -p on %COMPUTERNAME% to create directory to store preserved Prefetch files
		echo %DATE% %TIME% - Running mkdir -p on %COMPUTERNAME% to create directory to store preserved Prefetch files >> %c_drive%:\Data-%case%\Collection.log
		mkdir -p %c_drive%:\Data-%case%\%computername%-%timestamp%\preserved-files\Prefetch
		echo %DATE% %TIME% - Running mkdir -p on %COMPUTERNAME% to create directory for active profile (%USERPROFILE%) ntuser.dat hive
		echo %DATE% %TIME% - Running mkdir -p on %COMPUTERNAME% to directory for active profile (%USERPROFILE%) ntuser.dat hive >> %c_drive%:\Data-%case%\Collection.log
		mkdir -p %c_drive%:\Data-%case%\%computername%-%timestamp%\preserved-files\NTUSER_DAT
		echo %DATE% %TIME% - Running mkdir -p on %COMPUTERNAME% to create directory for RecentFileCache.bcf
		echo %DATE% %TIME% - Running mkdir -p on %COMPUTERNAME% to directory for RecentFileCache.bcf >> %c_drive%:\Data-%case%\Collection.log
		mkdir -p %c_drive%:\Data-%case%\%computername%-%timestamp%\preserved-files\AppCompat
	:: robocopy preserves the prefetch files 
		echo %DATE% %TIME% - Running robocopy.exe on %COMPUTERNAME% to preserve prefetch files
		echo %DATE% %TIME% - Running robocopy.exe on %COMPUTERNAME% to preserve prefetch files >> %c_drive%:\Data-%case%\Collection.log
		...
		robocopy.exe %WINDIR%\Prefetch %c_drive%:\Data-%case%\%computername%-%timestamp%\preserved-files\Prefetch\ *.pf /ZB /copy:DAT /r:0 /ts /FP /np /log:%c_drive%:\Data-%case%\%computername%-%timestamp%\preserved-files\pretch-robocopy-log.txt
	::RawCopy collects protected files from a live system including registry hives
		echo %DATE% %TIME% - Running RawCopy.exe on %COMPUTERNAME% to preserve active profile's ntuser.dat hive located in %USERPROFILE%
		echo %DATE% %TIME% - Running RawCopy.exe on %COMPUTERNAME% to preserve active profile's ntuser.dat hive located in %USERPROFILE% >> %c_drive%:\Data-%case%\Collection.log
		if %arch% == 32 (RawCopy.exe "%USERPROFILE%\NTUSER.DAT" %c_drive%:\Data-%case%\%computername%-%timestamp%\preserved-files\NTUSER_DAT) else (RawCopy64.exe "%USERPROFILE%\NTUSER.DAT" %c_drive%:\Data-%case%\%computername%-%timestamp%\preserved-files\NTUSER_DAT)
	::RawCopy collects protected files from a live system 
		echo %DATE% %TIME% - Running RawCopy.exe on %COMPUTERNAME% to preserve the RecentFileCache.bcf located in C:\Windows\AppCompat\Programs
		echo %DATE% %TIME% - Running RawCopy.exe on %COMPUTERNAME% to preserve the RecentFileCache.bcf located in C:\Windows\AppCompat\Programs >> %c_drive%:\Data-%case%\Collection.log
		if %arch% == 32 (RawCopy.exe %WINDIR%\AppCompat\Programs\RecentFileCache.bcf %c_drive%:\Data-%case%\%computername%-%timestamp%\preserved-files\AppCompat) else (RawCopy64.exe %WINDIR%\AppCompat\Programs\RecentFileCache.bcf %c_drive%:\Data-%case%\%computername%-%timestamp%\preserved-files\AppCompat)
...
:acquire_memory
	...
	:: Creating output folder for the memory image
		if not exist %c_drive%:\Data-%case%\%computername%-%timestamp%\memory-image (
			echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the memory image
			echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the memory image >> %c_drive%:\Data-%case%\Collection.log
			mkdir %c_drive%:\Data-%case%\%computername%-%timestamp%\memory-image
			)
	...
	echo %DATE% %TIME% - Running winpmem.exe on %COMPUTERNAME% to create memory image
	echo %DATE% %TIME% - Running winpmem.exe on %COMPUTERNAME% to create memory image >> %c_drive%:\Data-%case%\Collection.log
	winpmem.exe %mem_outpath%\physmem.bin
...
:acquire_volatile
	...
	if not exist %c_drive%:\Data-%case%\%computername%-%timestamp%\volatile-data (
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the volatile data
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the volatile data >> %c_drive%:\Data-%case%\Collection.log
		mkdir %c_drive%:\Data-%case%\%computername%-%timestamp%\volatile-data
		)
	...
	:: Collecting process information
		...
		:: pslist.exe reference: Malware Forensics page 35 or WFA page 26
			...
			echo %DATE% %TIME% - Running pslist.exe /accepteula on %COMPUTERNAME% to obtain the running processes
			echo %DATE% %TIME% - Running pslist.exe /accepteula on %COMPUTERNAME% to obtain the running processes >> %c_drive%:\Data-%case%\Collection.log
			pslist.exe /accepteula >> %vol_outpath%\ProcessInfo_1_running-processes.txt
		...
		:: cprocess.exe  /stext reference: Malware Forensics page 38
			echo %DATE% %TIME% - Running cprocess.exe  /stext on %COMPUTERNAME% to obtain the processes to user mapping
			echo %DATE% %TIME% - Running cprocess.exe  /stext on %COMPUTERNAME% to obtain the processes to user mapping >> %c_drive%:\Data-%case%\Collection.log	
			cprocess.exe  /stext %vol_outpath%\ProcessInfo_3_process-to-user-mapping.txt
		:: Listing the processes to user mapping in tab-delimited text file
		:: cprocess.exe  /stab reference: Malware Forensics page 38
			echo %DATE% %TIME% - Running cprocess.exe  /stab on %COMPUTERNAME% to obtain the processes to user mapping in tab-delimited text file
			echo %DATE% %TIME% - Running cprocess.exe  /stab on %COMPUTERNAME% to obtain the processes to user mapping in tab-delimited text file >> %c_drive%:\Data-%case%\Collection.log	
			cprocess.exe  /stab %vol_outpath%\ProcessInfo_3_process-to-user-mapping_tab.csv
		...
		:: pslist.exe -t reference: Malware Forensics page 40 or WFA page 26
			...
			pslist.exe -t /accepteula >> %vol_outpath%\ProcessInfo_4_child-processes.txt
		...
		:: handle.exe reference: Malware Forensics page 42 or WFA page 27
			...
			echo %DATE% %TIME% - Running handle.exe /accepteula on %COMPUTERNAME% to obtain the processes' file handles
			echo %DATE% %TIME% - Running handle.exe /accepteula on %COMPUTERNAME% to obtain the processes' file handles >> %c_drive%:\Data-%case%\Collection.log
			handle.exe /accepteula >> %vol_outpath%\ProcessInfo_5_processe-file-handles.txt
		...
		:: listdlls.exe reference: Malware Forensics page 44 or WFA page 26
			...
			echo %DATE% %TIME% - Running listdlls.exe /accepteula on %COMPUTERNAME% to obtain the processes' dependencies
			echo %DATE% %TIME% - Running listdlls.exe /accepteula on %COMPUTERNAME% to obtain the processes' dependencies >> %c_drive%:\Data-%case%\Collection.log
			listdlls.exe /accepteula >> %vol_outpath%\ProcessInfo_6_processe-dependencies.txt
	:: Collecting network information
		...
		:: openports.exe -lines -path reference: Malware Forensics page 49
			...
			echo %DATE% %TIME% - Running openports.exe -lines -path on %COMPUTERNAME% to obtain the port to process mapping grouped together
			echo %DATE% %TIME% - Running openports.exe -lines -path on %COMPUTERNAME% to obtain the port to process mapping grouped together >> %c_drive%:\Data-%case%\Collection.log
			openports.exe -lines -path >> %vol_outpath%\NetworkInfo_8_port-to-process-mapping_grouped.txt
		:: Listing the port to process mapping
		:: tcpvcon -a -c reference: WFA page 32
			echo %DATE% %TIME% - Running tcpvcon.exe -a -c /accepteula on %COMPUTERNAME% to obtain the port to process mapping in csv format
			echo %DATE% %TIME% - Running tcpvcon.exe -a -c /accepteula on %COMPUTERNAME% to obtain the port to process mapping in csv format >> %c_drive%:\Data-%case%\Collection.log
			tcpvcon.exe -a -c /accepteula >> %vol_outpath%\NetworkInfo_8_port-to-process-mapping_csv.csv
	:: Collecting logged on users information
		...
		:: psloggedon.exe reference: Malware Forensics page 24 or WFA page 17
			...
			echo %DATE% %TIME% - Running psloggedon.exe /accepteula on %COMPUTERNAME% to obtain the locally and remotely logged on users including those accessing resource shares
			echo %DATE% %TIME% - Running psloggedon.exe /accepteula on %COMPUTERNAME% to obtain the locally and remotely logged on users including those accessing resource shares >> %c_drive%:\Data-%case%\Collection.log
			psloggedon.exe /accepteula >> %vol_outpath%\UserInfo_1_locally-remotely-logged-on-users.txt
		...
		:: logonsessions.exe reference: Malware Forensics page 25 or WFA page 18
			...
			echo %DATE% %TIME% - Running logonsessions.exe /accepteula on %COMPUTERNAME% to obtain the active logon sessions
			echo %DATE% %TIME% - Running logonsessions.exe /accepteula on %COMPUTERNAME% to obtain the active logon sessions >> %c_drive%:\Data-%case%\Collection.log
			logonsessions.exe /accepteula >> %vol_outpath%\UserInfo_3_active-logon-sessions.txt
	:: Collecting opened files information
		...
		:: openedfilesview.exe /stext reference: Malware Forensics page 58 and WFA page 19
			echo %DATE% %TIME% - Running openedfilesview.exe /stext on %COMPUTERNAME% to obtain the open files on the computer
			echo %DATE% %TIME% - Running openedfilesview.exe /stext on %COMPUTERNAME% to obtain the open files on the computer >> %c_drive%:\Data-%case%\Collection.log	
			START /WAIT openedfilesview.exe /stext %vol_outpath%\OpenedFilesInfo_1_opened-files.txt
		...
		:: psfile.exe reference: Malware Forensics page 59 or WFA page 19
			...
			echo %DATE% %TIME% - Running psfile.exe /accepteula on %COMPUTERNAME% to obtain the remotely opened files
			echo %DATE% %TIME% - Running psfile.exe /accepteula on %COMPUTERNAME% to obtain the remotely opened files >> %c_drive%:\Data-%case%\Collection.log
			psfile.exe /accepteula >> %vol_outpath%\OpenedFilesInfo_2_remotely-opened-files.txt
	:: Collecting misc information
		...
		:: pclip.exe reference: Malware Forensics page 63 and WFA page 37
			...
			echo %DATE% %TIME% - Running pclip.exe on %COMPUTERNAME% to obtain the clipboard contents on the computer
			echo %DATE% %TIME% - Running pclip.exe on %COMPUTERNAME% to obtain the clipboard contents on the computer >> %c_drive%:\Data-%case%\Collection.log
			pclip.exe >> %vol_outpath%\MiscInfo_1_clipboard-contents.txt
	:: Collecting system information
		...
		:: uptime.exe reference: Malware Forensics page 21
			...
			echo %DATE% %TIME% - Running uptime.exe on %COMPUTERNAME% to obtain the system's uptime
			echo %DATE% %TIME% - Running uptime.exe on %COMPUTERNAME% to obtain the system's uptime >> %c_drive%:\Data-%case%\Collection.log
			uptime.exe >> %vol_outpath%\SystemInfo_2_system-uptime.txt
		...
		:: urlprotocolview.exe /stext reference: Malware Forensics page 20
			echo %DATE% %TIME% - Running urlprotocolview.exe /stext on %COMPUTERNAME% to obtain the enabled network protocols
			echo %DATE% %TIME% - Running urlprotocolview.exe /stext on %COMPUTERNAME% to obtain the enabled network protocols >> %c_drive%:\Data-%case%\Collection.log	
			urlprotocolview.exe /stext %vol_outpath%\SystemInfo_4_enabled-network-protocols.txt
		:: Listing the enabled network protocols
		:: urlprotocolview.exe /stab reference: Malware Forensics page 20
			echo %DATE% %TIME% - Running urlprotocolview.exe /stab on %COMPUTERNAME% to obtain the enabled network protocols in tab-delimited text file
			echo %DATE% %TIME% - Running urlprotocolview.exe /stab on %COMPUTERNAME% to obtain the enabled network protocolsin tab-delimited text file >> %c_drive%:\Data-%case%\Collection.log	
			urlprotocolview.exe /stab %vol_outpath%\SystemInfo_4_enabled-network-protocols_tab.csv
		...
		:: promiscdetect.exe reference: Malware Forensics page 19 and WFA page 35
			...
			echo %DATE% %TIME% - Running promiscdetect.exe on %COMPUTERNAME% to obtain the network adapters in promiscuous mode
			echo %DATE% %TIME% - Running promiscdetect.exe on %COMPUTERNAME% to obtain the network adapters in promiscuous mode >> %c_drive%:\Data-%case%\Collection.log
			promiscdetect.exe >> %vol_outpath%\SystemInfo_5_promiscuous-adapters.txt	
	echo:
...
:acquire_nonvolatile
	...
	:: Creating output directory
	if not exist %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data (
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the non-volatile data
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the non-volatile data >> %c_drive%:\Data-%case%\Collection.log
		mkdir %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data
		)
	::Creating output directories
	if not exist %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\mbr (
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the MBR non-volatile data
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the MBR non-volatile data >> %c_drive%:\Data-%case%\Collection.log
		mkdir %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\mbr
		)
		if not exist %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\registry (
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the registry hives
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the registry hives >> %c_drive%:\Data-%case%\Collection.log
		mkdir %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\registry
		)
		if not exist %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\ntfs (
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the NTFS artifacts
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the NTFS artifacts >> %c_drive%:\Data-%case%\Collection.log
		mkdir %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\ntfs
		)
		if not exist %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\autoruns (
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the autoruns non-volatile data
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the autoruns non-volatile data >> %c_drive%:\Data-%case%\Collection.log
		mkdir %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\autoruns
		)
		if not exist %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\logs (
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store log files
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store log files >> %c_drive%:\Data-%case%\Collection.log
		mkdir %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\logs
		)
		if not exist %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\group-policy (
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the group-policy non-volatile data
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store the group-policy non-volatile data >> %c_drive%:\Data-%case%\Collection.log
		mkdir %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\group-policy
		)
	:: Creating a variable to the volatile-data directory to shorten the code
	...
	:: Collecting the System's Boot Record Information
		...
		echo %DATE% %TIME% - Running mmls.exe \\.\PHYSICALDRIVE0 on %COMPUTERNAME% to document the partition information
		echo %DATE% %TIME% - Running mmls.exe \\.\PHYSICALDRIVE0 on %COMPUTERNAME% to document the partition information >> %c_drive%:\Data-%case%\Collection.log
		mmls.exe \\.\PHYSICALDRIVE0 >> %nonvol_outpath%\mbr\%COMPUTERNAME%_partition-info.txt
		:: Imaging the MBR in sector 0
		echo %DATE% %TIME% - Running dd.exe if=\\.\PHYSICALDRIVE0 of=path-mbr.bin bs=512 count=1 on %COMPUTERNAME% to document obtain the MBR
		echo %DATE% %TIME% - Runningdd.exe if=\\.\PHYSICALDRIVE0 of=path-mbr.bin bs=512 count=1 on %COMPUTERNAME% to document obtain the MBR >> %c_drive%:\Data-%case%\Collection.log
		dd.exe if=\\.\PHYSICALDRIVE0 of=%nonvol_outpath%\mbr\%COMPUTERNAME%_mbr.bin bs=512 count=1
		:: Imaging the sectors before the first partition
		echo %DATE% %TIME% - Running dd.exe if=\\.\PHYSICALDRIVE0 of=path-bytes.bin  bs=512 count=63 or count-2048 on %COMPUTERNAME% to obtain sectors before first partition
		echo %DATE% %TIME% - Running dd.exe if=\\.\PHYSICALDRIVE0 of=path-bytes.bin  bs=512 count=63 or count-2048 on %COMPUTERNAME% to obtain sectors before first partition >> %c_drive%:\Data-%case%\Collection.log
		if %os% == legacy (dd.exe if=\\.\PHYSICALDRIVE0 of=%nonvol_outpath%\mbr\winXP_2003-63-bytes.bin bs=512 count=63) else (dd.exe if=\\.\PHYSICALDRIVE0 of=%nonvol_outpath%\mbr\win7_2008-2048-bytes.bin bs=512 count=2048)
	:: Collecting the System's Registry Hives
		:: Collecting the registry hives in the config folder
		echo %DATE% %TIME% - Running RawCopy.exe on %COMPUTERNAME% to obtain the SAM, SECURITY, SOFTWARE, and SYSTEM registry hives
		echo %DATE% %TIME% - Running RawCopy.exe on %COMPUTERNAME% to obtain the SAM, SECURITY, SOFTWARE, and SYSTEM registry hives >> %c_drive%:\Data-%case%\Collection.log
		if %arch% == 32 (
			RawCopy.exe %WINDIR%\System32\config\SAM %nonvol_outpath%\registry
			RawCopy.exe %WINDIR%\System32\config\SECURITY %nonvol_outpath%\registry
			RawCopy.exe %WINDIR%\System32\config\SOFTWARE %nonvol_outpath%\registry
			RawCopy.exe %WINDIR%\System32\config\SYSTEM %nonvol_outpath%\registry
			)
		if %arch% == 64 (
			RawCopy64.exe %WINDIR%\System32\config\SAM %nonvol_outpath%\registry
			RawCopy64.exe %WINDIR%\System32\config\SECURITY %nonvol_outpath%\registry
			RawCopy64.exe %WINDIR%\System32\config\SOFTWARE %nonvol_outpath%\registry
			RawCopy64.exe %WINDIR%\System32\config\SYSTEM %nonvol_outpath%\registry
			)
		if  NOT %os% == legacy (
			if not exist %nonvol_outpath%\registry\RegBack (
				echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create the RegBack directory
				echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create the RegBack directory >> %c_drive%:\Data-%case%\Collection.log
				mkdir %nonvol_outpath%\registry\RegBack
			)
			echo %DATE% %TIME% - Running RawCopy.exe on %COMPUTERNAME% to obtain the SAM, SECURITY, SOFTWARE, and SYSTEM registry hives in RegBack folder
			echo %DATE% %TIME% - Running RawCopy.exe on %COMPUTERNAME% to obtain the SAM, SECURITY, SOFTWARE, and SYSTEM registry hives in RegBack folder >> %c_drive%:\Data-%case%\Collection.log
			if %arch% == 32 (
				RawCopy.exe %WINDIR%\System32\config\RegBack\SAM %nonvol_outpath%\registry\RegBack
				RawCopy.exe %WINDIR%\System32\config\RegBack\SECURITY %nonvol_outpath%\registry\RegBack
				RawCopy.exe %WINDIR%\System32\config\RegBack\SOFTWARE %nonvol_outpath%\registry\RegBack
				RawCopy.exe %WINDIR%\System32\config\RegBack\SYSTEM %nonvol_outpath%\registry\RegBack
				RawCopy.exe %WINDIR%\System32\config\RegBack\SYSTEM %nonvol_outpath%\registry\RegBack
			)
			if %arch% == 64 (
				RawCopy64.exe %WINDIR%\System32\config\RegBack\SAM %nonvol_outpath%\registry\RegBack
				RawCopy64.exe %WINDIR%\System32\config\RegBack\SECURITY %nonvol_outpath%\registry\RegBack
				RawCopy64.exe %WINDIR%\System32\config\RegBack\SOFTWARE %nonvol_outpath%\registry\RegBack
				RawCopy64.exe %WINDIR%\System32\config\RegBack\SYSTEM %nonvol_outpath%\registry\RegBack
				RawCopy64.exe %WINDIR%\System32\config\RegBack\DEFAULT %nonvol_outpath%\registry\RegBack
			)
		)
	:: Collecting each user's registry hive
		...
		for /f "tokens=*" %%i in ('dir /ah /b /s ntuser.dat') do @for /f "tokens=3 delims=\" %%j in ("%%i") do @for /f "tokens=4 delims=\" %%h in ("%%i") do (
			if not exist %nonvol_outpath%\registry\%%j (
				echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create the collection for the registry\%%j folder
				echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create the collection for the registry\%%j folder >> %c_drive%:\Data-%case%\Collection.log
				"%script_path%\mkdir" %nonvol_outpath%\registry\%%j
			)
			echo %DATE% %TIME% - Running RawCopy.exe %%i on %COMPUTERNAME% to extract the NTUSER.DAT for %%j
			echo %DATE% %TIME% - Running RawCopy.exe %%i on %COMPUTERNAME% to extract the NTUSER.DAT for %%j  >> %c_drive%:\Data-%case%\Collection.log
			if %arch% == 32 ("%script_path%\RawCopy.exe" "%%i" %nonvol_outpath%\registry\%%j)
			if %arch% == 64 ("%script_path%\RawCopy64.exe" "%%i" %nonvol_outpath%\registry\%%j)
		)
		...
		if NOT %os% == legacy (
			...
			echo %DATE% %TIME% - Running RawCopy.exe %%i on %COMPUTERNAME% to extract the UsrClass.dat for %%j
			echo %DATE% %TIME% - Running RawCopy.exe %%i on %COMPUTERNAME% to extract the UsrClass.dat for %%j  >> %c_drive%:\Data-%case%\Collection.log
			if %arch% == 32 ("%script_path%\RawCopy.exe" "%%i" %nonvol_outpath%\registry\%%j)
			if %arch% == 64 ("%script_path%\RawCopy64.exe" "%%i" %nonvol_outpath%\registry\%%j)
			)
		)
	...
	:: Collecting NTFS artifacts
		...
		echo %DATE% %TIME% - Running RawCopy.exe %SYSTEMDRIVE%\$MFT on %COMPUTERNAME% to extract the $MFT
		echo %DATE% %TIME% - Running RawCopy.exe %SYSTEMDRIVE%\$MFT on %COMPUTERNAME% to extract the $MFT  >> %c_drive%:\Data-%case%\Collection.log
		if %arch% == 32 (RawCopy.exe %SYSTEMDRIVE%0 %nonvol_outpath%\ntfs) else (RawCopy64.exe %SYSTEMDRIVE%0 %nonvol_outpath%\ntfs)
		:: Collecting the $LogFile record
		echo %DATE% %TIME% - Running RawCopy.exe %SYSTEMDRIVE%\$LogFile on %COMPUTERNAME% to extract the $LogFile
		echo %DATE% %TIME% - Running RawCopy.exe %SYSTEMDRIVE%\$LogFile on %COMPUTERNAME% to extract the $LogFile  >> %c_drive%:\Data-%case%\Collection.log
		if %arch% == 32 (RawCopy.exe %SYSTEMDRIVE%2 %nonvol_outpath%\ntfs) else (RawCopy64.exe %SYSTEMDRIVE%2 %nonvol_outpath%\ntfs)
	:: Collecting autostarting locations
		...
		:: autorunsc.exe reference: Malware Forensics page 69 or WFA page 44
			...
			echo %DATE% %TIME% - Running autorunsc.exe -a -v /accepteula on %COMPUTERNAME% to obtain the autostarting locations
			echo %DATE% %TIME% - Running autorunsc.exe -a -v /accepteula on %COMPUTERNAME% to obtain the autostarting locations >> %c_drive%:\Data-%case%\Collection.log
			autorunsc.exe -a /accepteula >> %nonvol_outpath%\autoruns\%COMPUTERNAME%-autostarting-locations.txt
		:: Listing the system's autostarting locations in csv format
		:: autorunsc.exe reference: Malware Forensics page 69 or WFA page 44
			echo %DATE% %TIME% - Running autorunsc.exe -a -c -v /accepteula on %COMPUTERNAME% to obtain the autostarting locations in csv format
			echo %DATE% %TIME% - Running autorunsc.exe -a -c -v /accepteula on %COMPUTERNAME% to obtain the autostarting locations in csv format >> %c_drive%:\Data-%case%\Collection.log
			autorunsc.exe -a -c /accepteula >> %nonvol_outpath%\autoruns\%COMPUTERNAME%-autostarting-locations_csv.csv
		...
		:: Collecting scheduled task log and/or folder
			...
			if %os% == legacy (
				if %arch% == 32 (RawCopy.exe %WINDIR%\SchedLgU.txt %nonvol_outpath%\autoruns)
				if %arch% == 64 (RawCopy64.exe %WINDIR%\SchedLgU.txt %nonvol_outpath%\autoruns)
			) else (
				echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store tasks
				echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store tasks >> %c_drive%:\Data-%case%\Collection.log
				mkdir %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\autoruns\Tasks_folder
				echo %DATE% %TIME% - Running robocopy.exe on %COMPUTERNAME% to collect the Task folder
				echo %DATE% %TIME% - Running robocopy.exe on %COMPUTERNAME% to collect the Task folder >> %c_drive%:\Data-%case%\Collection.log
				:: The following are the robocopy swithes: /zb  Tries to copy files in restartable mode, 
				::  /copy:DAT copy file data, timestamps, and attribute /r:0  retry 0 times,
				:: /ts source timestamps in log, /FP displays full pathnames in output,
				:: /np progress indicator turned off and /log  creates log file by overwriting one if already exists
				robocopy.exe %WINDIR%\Tasks %nonvol_outpath%\autoruns\Tasks_folder\ /ZB /copy:DAT /r:0 /ts /FP /np /log:%nonvol_outpath%\autoruns\tasks-robocopy-log.txt
				)
	...
	:: Collecting log files
		...
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store event logs
		echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store event logs >> %c_drive%:\Data-%case%\Collection.log
		mkdir %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\logs\event-logs
		)
		if %os% == legacy (
			echo %DATE% %TIME% - Running RawCopy.exe on %COMPUTERNAME% to extract the AppEvent.Evt, SecEvent.Evt, and SysEvent.Evt logs
			echo %DATE% %TIME% - Running RawCopy.exe on %COMPUTERNAME% to extract the AppEvent.Evt, SecEvent.Evt, and SysEvent.Evt logs  >> %c_drive%:\Data-%case%\Collection.log
			if %arch% == 32 (
				RawCopy.exe %WINDIR%\System32\config\AppEvent.Evt %nonvol_outpath%\logs\event-logs
				RawCopy.exe %WINDIR%\System32\config\SecEvent.Evt %nonvol_outpath%\logs\event-logs
				RawCopy.exe %WINDIR%\System32\config\SysEvent.Evt %nonvol_outpath%\logs\event-logs
			)
			if %arch% == 64 (
				RawCopy64.exe %WINDIR%\System32\config\AppEvent.Evt %nonvol_outpath%\logs\event-logs
				RawCopy64.exe %WINDIR%\System32\config\SecEvent.Evt %nonvol_outpath%\logs\event-logs
				RawCopy64.exe %WINDIR%\System32\config\SysEvent.Evt %nonvol_outpath%\logs\event-logs
			)
		) else (
			echo %DATE% %TIME% - Running RawCopy.exe on %COMPUTERNAME% to collect the event logs
			echo %DATE% %TIME% - Running RawCopy.exe on %COMPUTERNAME% to collect the event logs >> %c_drive%:\Data-%case%\Collection.log
			if %arch% == 32 (
				RawCopy.exe %WINDIR%\System32\winevt\Logs\Application.evtx %nonvol_outpath%\logs\event-logs
				RawCopy.exe %WINDIR%\System32\winevt\Logs\Security.evtx %nonvol_outpath%\logs\event-logs
				RawCopy.exe %WINDIR%\System32\winevt\Logs\System.evtx %nonvol_outpath%\logs\event-logs
			)
			if %arch% == 64 (
				RawCopy64.exe %WINDIR%\System32\winevt\Logs\Application.evtx %nonvol_outpath%\logs\event-logs
				RawCopy64.exe %WINDIR%\System32\winevt\Logs\Security.evtx %nonvol_outpath%\logs\event-logs
				RawCopy64.exe %WINDIR%\System32\winevt\Logs\System.evtx %nonvol_outpath%\logs\event-logs
			)
		)
		:: Collecting the Log folder for non Windows legacy systems
 		if /I not %os% == legacy (
			echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store Logs folder
			echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store Logs folder >> %c_drive%:\Data-%case%\Collection.log
			mkdir %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\logs\Logs_folder
			echo %DATE% %TIME% - Running robocopy.exe on %COMPUTERNAME% to collect the Log folder
			echo %DATE% %TIME% - Running robocopy.exe on %COMPUTERNAME% to collect the Log folder >> %c_drive%:\Data-%case%\Collection.log
			:: The following are the robocopy swithes: /zb  Tries to copy files in restartable mode, 
			...
			:: /np progress indicator turned off and /log  creates log file by overwriting one if already exists
			robocopy.exe %SYSTEMDRIVE%\Windows\Logs\ %nonvol_outpath%\logs\Logs_folder /ZB /copy:DAT /r:0 /ts /FP /np /E /log:%nonvol_outpath%\logs\logs-robocopy-log.txt 
		)
		:: Collecting the McAfee log and quarantine folders
		if not exist %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\logs\McAfee (
			echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store McAfee log folder
			echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store McAfee log folder >> %c_drive%:\Data-%case%\Collection.log
			mkdir %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\logs\McAfee
		)
		if not exist %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\logs\McAfee\Quarantine (
			echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store McAfee Quarantine folder
			echo %DATE% %TIME% - Running mkdir on %COMPUTERNAME% to create directory to store McAfee Quarantine folder >> %c_drive%:\Data-%case%\Collection.log
			mkdir %c_drive%:\Data-%case%\%computername%-%timestamp%\nonvolatile-data\logs\McAfee\Quarantine
		)
		if %os% == legacy (
			echo %DATE% %TIME% - Running robocopy.exe on %COMPUTERNAME% to collect the McAfee log folder
			echo %DATE% %TIME% - Running robocopy.exe on %COMPUTERNAME% to collect the McAfee log folder >> %c_drive%:\Data-%case%\Collection.log
			:: The following are the robocopy swithes: /zb  Tries to copy files in restartable mode, 
			::  /copy:DAT copy file data, timestamps, and attribute /r:0  retry 0 times,
			:: /ts source timestamps in log, /FP displays full pathnames in output, /E copy subfolders
			:: /np progress indicator turned off and /log  creates log file by overwriting one if already exists
			robocopy.exe "%SYSTEMDRIVE%\Documents and Settings\All Users\Application Data\McAfee\DesktopProtection" %nonvol_outpath%\logs\McAfee /ZB /copy:DAT /r:0 /ts /FP /np /E /log:%nonvol_outpath%\logs\logs-robocopy-log.txt
			echo %DATE% %TIME% - Running robocopy.exe on %COMPUTERNAME% to collect the McAfee Quarantine folder
			echo %DATE% %TIME% - Running robocopy.exe on %COMPUTERNAME% to collect the McAfee Quarantine folder >> %c_drive%:\Data-%case%\Collection.log
			:: The following are the robocopy swithes: /zb  Tries to copy files in restartable mode, 
			::  /copy:DAT copy file data, timestamps, and attribute /r:0  retry 0 times,
			:: /ts source timestamps in log, /FP displays full pathnames in output, /E copy subfolders
			:: /np progress indicator turned off and /log  creates log file by overwriting one if already exists
			robocopy.exe "%SYSTEMDRIVE%\Documents and Settings\All Users\Application Data\McAfee\Quarantine" %nonvol_outpath%\logs\McAfee\Quarantine /ZB /copy:DAT /r:0 /ts /FP /np /E /log:%nonvol_outpath%\logs\McAfee\quarantine-robocopy-log.txt
			) else (
				echo %DATE% %TIME% - Running robocopy.exe on %COMPUTERNAME% to collect the McAfee log folder
				echo %DATE% %TIME% - Running robocopy.exe on %COMPUTERNAME% to collect the McAfee log folder >> %c_drive%:\Data-%case%\Collection.log
				:: The following are the robocopy swithes: /zb  Tries to copy files in restartable mode, 
				...
				:: /ts source timestamps in log, /FP displays full pathnames in output, /E copy subfolders
				:: /np progress indicator turned off and /log  creates log file by overwriting one if already exists
				robocopy.exe %SYSTEMDRIVE%\ProgramData\McAfee\DesktopProtection %nonvol_outpath%\logs\McAfee /ZB /copy:DAT /r:0 /ts /FP /np /E /log:%nonvol_outpath%\logs\logs-robocopy-log.txt
				echo %DATE% %TIME% - Running robocopy.exe on %COMPUTERNAME% to collect the McAfee Quarantine folder
				echo %DATE% %TIME% - Running robocopy.exe on %COMPUTERNAME% to collect the McAfee Quarantine folder >> %c_drive%:\Data-%case%\Collection.log
				:: The following are the robocopy swithes: /zb  Tries to copy files in restartable mode, 
				:: /copy:DAT copy file data and timestamps /r:0  retry 0 times,
				...
				:: /np progress indicator turned off and /log  creates log file by overwriting one if already exists
			:: McAfee logs
			::	robocopy.exe %SYSTEMDRIVE%\ProgramData\McAfee\Virusscan\Quarantine %nonvol_outpath%\logs\McAfee\Quarantine /ZB /copy:DAT /r:0 /ts /FP /np /E /log:%nonvol_outpath%\logs\McAfee\quarantine-robocopy-log.txt
			:: Malicious Software Removal Toolkit logs
			robocopy.exe /ZB /copy:DAT /r:0 /ts /FP /np /E %SystemRoot%\debug\mrt.log %nonvol_outpath%\logs\Microsoft\
			)
	:: Collecting the group policy information applied to the system
		...
		:: gplist.exe reference: Malware Forensics page 73
			...
			echo %DATE% %TIME% - Running gplist.exe on %COMPUTERNAME% to obtain the computer's group policy listing 
			echo %DATE% %TIME% - Running gplist.exe on %COMPUTERNAME% to obtain the computer's group policy listing >> %c_drive%:\Data-%case%\Collection.log
			gplist.exe >> %nonvol_outpath%\group-policy\%COMPUTERNAME%-group-policy-listing.txt
	...
	:tr3ps
	PowerShell -NoProfile -ExecutionPolicy Bypass -Noninteractive -WindowStyle Hidden tr3.ps1 -methodParam smb -collectrCfgFileParam Extended_Collection
	robocopy %c_drive%:\Data-%case%\ /ZB /copy:DAT /r:0 /ts /FP /np "c:\Program Files (x86)\Tanium\Tanium Client\collect"
	goto :exit
:: --------------------------------------------------------------------------------------------------------------------------
