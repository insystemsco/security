...
.DESCRIPTION
This tool pulls data from a target Windows 7 or later systems where there is suspicious of misuse and/or infection. This will extract useful forensic data that will assist IR teams in gathering quick live data on a potentially compromised host.


.EXAMPLE
    PS C:\> .\TR3PS.ps1
        Simply run TR3PS on the local host.
        This gathers default data and stores the results in the directory that the script was executed from.

.EXAMPLE
    PS C:\> .\TR3PS.ps1 -remote -target [computer] [arguments - EX: -sendEmail -share -username -password]
        Run TR3PS Remotely.
...
.EXAMPLE
    PS C:\> .\TR3PS.ps1 -sendEmail -smtpServer ["127.0.0.1"] -emailTo ["greg.foss[at]logrhythm.com"] -emailFrom ["TR3PS[at]logrhythm.com"]
...
.EXAMPLE
    PS C:\> .\TR3PS.ps1 -share -netShare ["\\share\"] -Credential Get-Credential
...
.EXAMPLE
    PS C:\> .\TR3PS.ps1 -lockdown -adLock [username]
...
.EXAMPLE
    PS C:\> .\TR3PS.ps1 -username ["admin user"] -password ["pass"]
        ...
        These parameters are used when running TR3PS on remote hosts or interacting with Active Directory; not required for local execution.
        If neither parameter is supplied, you will be prompted for credentials // safest option aside from local execution

.EXAMPLE
    Remotely enable PSRemoting and Unrestricted PowerShell Execution then, run TR3PS.
    First, enable PSRemoting
        ...
        PS C:\> .\TR3PS.ps1 -remote -target "10.10.10.10" -sendEmail -smtpServer "127.0.0.1" -emailTo "greg.foss[at]logrhythm.com" -emailFrom "TR3PS[at]logrhythm.com"
...
"@
    ...
    Write-Host "     This option was specified "
    Write-Host "PS C:\> .\TR3PS.ps1 -sendEmail -smtpServer ['SMTP SERVER IP'] -emailTo ['greg.foss[at]logrhythm.com'] -emailFrom ['TR3PS[at]logrhythm.com']"
    Write-EventLog -LogName Application -Source "TR3PS" -EntryType Information -EventId 34404 -Message "Forensic Data Acquisition Failure : Missing Required Parameter"
    Exit 1
    }
